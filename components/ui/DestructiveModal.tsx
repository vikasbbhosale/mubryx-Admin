...
interface DestructiveModalProps {
  ...
  description: string;
  confirmText?: string;
  confirmKeyword?: string; // If provided, user must type this exact word (e.g. 'CANCEL', 'SUSPEND')
...
export function DestructiveModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Dialog */}
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-xl shadow-2xl p-6 text-slate-100 z-10 space-y-4">
        {/* Header */}
        ...
        {consequences.length > 0 && (
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-lg p-3 text-xs space-y-1.5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {requireReason && (
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500/30"
              />
            </div>
          )}
          ...
          {confirmKeyword && (
            <div>
              <label className="block text-[11px] font-semibold text-slate-300 mb-1.5">
                Type <span className="font-mono text-rose-400 font-bold">{confirmKeyword}</span> to confirm:
              </label>
              <input
                type="text"
                value={typedKeyword}
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs font-mono text-white placeholder-slate-600 focus:outline-none focus:border-rose-500/50"
              />
            </div>
          )}
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="px-3.5 py-1.5 rounded-lg text-xs font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
