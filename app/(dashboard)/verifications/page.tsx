...
export default function VerificationsPage() {
  ...
  const fetchQueue = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setError(e?.message || 'Error fetching verification records from workforce API');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <div className="space-y-4 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-auto">
          ...
          >
            <RefreshCw className={`w-3.5 h-3.5 ${refreshing ? 'animate-spin text-amber-500 dark:text-amber-400' : ''}`} />
            <span>Sync</span>
          </button>
          <Link
            href="/technicians"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 transition-colors shadow-xs"
          >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg pl-9 pr-3 py-1.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500"
          />
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="py-2.5 px-4 font-mono text-[11px]">
                      {t.documents && t.documents.length > 0 ? (
                        ...
                        <span className="text-rose-600 dark:text-rose-400 font-semibold">0 documents</span>
                      )}
                    </td>

                    <td className="py-2.5 px-4 text-slate-500 dark:text-slate-400">
                      {t.bankDetails?.bankName ? (
                        <span className="truncate block max-w-[120px] text-slate-700 dark:text-slate-300 font-mono text-[11px]">
                          {t.bankDetails.bankName}
                        </span>
                      ) : (
                        <span className="text-slate-400 dark:text-slate-600 text-[11px]">Missing</span>
                      )}
                    </td>
            ...
            <>
              {selectedTech.onboardingStatus !== 'REJECTED' && (
                <button
                  ...
                  className="px-3.5 py-1.5 text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-lg transition-colors cursor-pointer"
                >
                  Reject
                </button>
              )}
          ...
          <div className="space-y-5">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2 text-slate-700 dark:text-slate-300">
                <div>
                  <span className="text-slate-500 dark:text-slate-500 block text-[11px]">Applicant Name</span>
                  <span className="font-semibold text-slate-900 dark:text-white">{selectedTech.fullName}</span>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-500 block text-[11px]">Contact Phone</span>
                  <span className="font-mono text-slate-900 dark:text-white">{selectedTech.contact || selectedTech.user?.phone}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3 text-xs">
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-800/60">
                {!selectedTech.documents || selectedTech.documents.length === 0 ? (
                  <p className="text-rose-600 dark:text-rose-400 py-2">
                  ...
                  selectedTech.documents.map((doc) => (
                    <div key={doc.id} className="py-2.5 flex items-center justify-between">
                      <div>
                        <span className="font-mono font-bold text-slate-900 dark:text-white block">{doc.type}</span>
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 transition-colors text-xs font-medium inline-flex items-center gap-1 cursor-pointer"
                      >
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500"
              />
            </div>
          </div>
        )}
