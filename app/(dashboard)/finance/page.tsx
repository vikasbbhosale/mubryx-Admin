...
export default function FinancePage() {
  ...
  const handleConfirmRefund = async () => {
    ...
    try {
      // Simulate backend refund flow or call backend if available
      await new Promise(r => setTimeout(r, 1000));
      setRefundSuccessMessage(`Refund of ₹${refundForm.amount} queued for Booking #${refundForm.bookingId}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl font-bold tracking-tight text-white">Financials & Commission Operations</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/30 rounded text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
          >
      ...
      {refundSuccessMessage && (
        <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-4 py-2.5 rounded-lg text-xs flex items-center justify-between">
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-lg p-3.5 relative overflow-hidden">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-lg p-3.5 relative overflow-hidden">
          ...
          <div className="text-[11px] text-emerald-500/80 mt-1">
            10–15% fee retained per fulfilled order
          </div>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-lg p-3.5 relative overflow-hidden">
          ...
          <div className="text-2xl font-bold font-mono text-indigo-300 mt-1.5">
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-lg overflow-hidden">
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3 px-4 text-slate-400 text-[11px]">
                        ₹{tx.balanceBefore.toFixed(2)} → <span className="text-white font-semibold">₹{tx.balanceAfter.toFixed(2)}</span>
                      </td>

                      {/* Description & Reference */}
                      <td className="py-3 px-4 font-sans">
                        <div className="text-slate-200 font-medium truncate max-w-xs">{tx.description || 'Ledger event'}</div>
                        {tx.referenceId && (
                          <div className="text-[10px] font-mono text-slate-500">Ref: {tx.referenceId}</div>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-4 text-right font-sans">
                        <button
                          ...
                          className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 transition-colors inline-flex items-center gap-1 cursor-pointer"
                        >
        ...
        <div className="px-4 py-2.5 bg-slate-950/60 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
          ...
          <span className="text-[11px] font-mono text-slate-500">Gateway: Razorpay Enterprise Route Compliant</span>
        </div>
      </div>
      ...
      <CaseDrawer
        ...
        status={activeTx?.type}
      >
        {activeTx && (
          <div className="space-y-4 text-xs font-mono">
            {/* Summary Banner */}
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 space-y-1">
              <div className="text-[11px] font-sans text-slate-400">Movement Amount</div>
              <div className="text-2xl font-bold text-white">
                ₹{activeTx.amount.toFixed(2)}
              </div>
              <div className="text-[11px] text-slate-500 font-sans">{activeTx.description}</div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 space-y-2">
              ...
              <div className="flex justify-between border-b border-slate-800 pb-2">
                ...
                <span className="text-slate-300">₹{activeTx.balanceBefore.toFixed(2)}</span>
              </div>
              <div className="flex justify-between pt-1">
                <span className="text-slate-500">Balance Subsequent:</span>
                <span className="text-emerald-400 font-bold">₹{activeTx.balanceAfter.toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 space-y-2">
              ...
              <div className="space-y-1.5">
                <div>
                  ...
                  <span className="text-slate-200 select-all">{activeTx.id}</span>
                </div>
                {activeTx.referenceId && (
                  <div>
                    <span className="text-slate-500 block">Linked Order / Reference ID:</span>
                    <span className="text-blue-400 select-all">{activeTx.referenceId}</span>
                  </div>
                )}
                <div>
      ...
      {isRefundModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-lg w-full p-5 space-y-4 shadow-2xl text-xs">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                <h3 className="text-sm font-bold text-white flex items-center gap-1.5">
                  <AlertTriangle className="w-4 h-4 text-rose-400" />
                  Issue Financial Refund / Adjustment
                </h3>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Step {refundStep} of 2: {refundStep === 1 ? 'Configure Refund Parameters' : 'Review & Authorize Ledger Movement'}
                </p>
              </div>
              <button
            ...
            {refundStep === 1 ? (
              <form onSubmit={handleInitiateRefund} className="space-y-3.5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-slate-700 font-mono"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-slate-700 font-mono font-bold"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-slate-700"
                  >
                ...
                <div className="space-y-2 pt-1 border-t border-slate-800">
                  <label className="flex items-center gap-2 cursor-pointer text-slate-300">
                    ...
                    />
                    <span>Deduct equivalent payout from assigned technician wallet</span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-slate-300">
                    ...
                    />
                    <span>Transmit SMS/WhatsApp notification to customer</span>
                  </label>
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-800">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-1.5 bg-rose-600 hover:bg-rose-500 text-white font-semibold rounded flex items-center gap-1 cursor-pointer"
                  >
              ...
              <div className="space-y-4">
                <div className="bg-rose-500/10 border border-rose-500/20 rounded p-3 text-rose-300 space-y-1">
                  <div className="font-bold flex items-center gap-1.5">
                    <AlertTriangle className="w-4 h-4 text-rose-400" />
                    IRREVERSIBLE FINANCIAL ACTION
                  </div>
                  <p className="text-[11px] text-rose-300/80">
                    Once authorized, ₹{refundForm.amount} will be marked for credit back to customer via original payment method. An immutable audit record will be logged with your administrator credentials.
                  </p>
                </div>

                <div className="bg-slate-950 border border-slate-800 rounded p-3 space-y-1.5 font-mono">
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-sans">Booking ID:</span>
                    <span className="text-white font-bold">{refundForm.bookingId}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-sans">Refund Amount:</span>
                    <span className="text-rose-400 font-bold">₹{Number(refundForm.amount).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-sans">Reason:</span>
                    <span className="text-slate-300 font-sans">{refundForm.reason}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-sans">Deduct Tech:</span>
                    <span className={refundForm.deductTechnician ? 'text-emerald-400' : 'text-slate-500'}>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-800">
                  ...
                  <button
                    ...
                    className="px-4 py-1.5 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
                    {refundProcessing && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                    <span>Confirm & Authorize ₹{refundForm.amount}</span>
                  </button>
                </div>
              </div>
            )}
